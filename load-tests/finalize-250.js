...
export const options={
 setupTimeout:'10m',
 scenarios:{finalize_250:{executor:'per-vu-iterations',vus:COMPLETIONS,iterations:1,maxDuration:'3m'}},
 thresholds:{
  'http_req_failed{phase:finalize}':['rate<0.01'],
  'http_req_duration{phase:finalize}':['p(95)<5000'],
  checks:['rate>0.99']
 }
};
...
export function setup(){
 ...
 const qToken=fixtures[0].token;
 const qr=http.get(`${SUPABASE_URL}/rest/v1/assessment_questions?is_active=eq.true&select=id&order=question_number.asc`,{headers:authHeaders(qToken),tags:{phase:'setup'}});
 ...
 if(attempts.length!==COMPLETIONS)throw new Error('Completion fixture validation failed');
 return{attempts};
}

export default function(data){
 const f=data.attempts[__VU-1];
 const r=http.post(`${SUPABASE_URL}/rest/v1/rpc/finalize_assessment`,JSON.stringify({p_attempt_id:f.attemptId}),{headers:authHeaders(f.token),tags:{phase:'finalize',operation:'finalize_assessment'}});
 check(r,{'assessment finalization succeeds':x=>x.status>=200&&x.status<300,'finalization returns recommendations':x=>{try{const b=x.json();return Array.isArray(b)&&b.length>0}catch{return false}}});
}
