...
export async function POST(request) {
  try {
    ...
    if (!SERVICE_ROLE_KEY)
      return NextResponse.json(
        { error: "Institution synchronization is not configured." },
        { status: 503 },
      );
    ...
    if (raw.length > 500)
      return NextResponse.json(
        {
          error:
            "A maximum of 500 institution associations may be synced at once.",
        },
        { status: 400 },
      );
      ...
      return NextResponse.json(
        {
          error:
            saved?.message ||
            saved?.hint ||
            "Unable to save institution associations.",
        },
    ...
    return NextResponse.json({
      ok: true,
      synced: Array.isArray(saved) ? saved.length : assignments.length,
    });
  } catch (error) {
    return NextResponse.json(
      { error: error?.message || "Unable to sync institution associations." },
      { status: 500 },
    );
  }
}
