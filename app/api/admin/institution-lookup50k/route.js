...
async function authorize(request) {
  ...
  const ar = await fetch(
    `${SUPABASE_URL}/rest/v1/admin_users?user_id=eq.${encodeURIComponent(me.id)}&select=user_id&limit=1`,
    { headers: serviceHeaders(), cache: "no-store" },
  );
  const admins = await readJson(ar);
  if (!ar.ok || !admins?.length)
    return {
      error: NextResponse.json(
        { error: "Administrator access is not enabled for this account." },
        { status: 403 },
      ),
    };
  return { me };
    ...
    for (let i = 0; i < emails.length; i += 20) {
      ...
      if (!r.ok)
        throw new Error(
          rows?.message ||
            rows?.error ||
            "Unable to load institution associations.",
        );
