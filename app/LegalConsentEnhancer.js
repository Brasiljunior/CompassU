...
export default function LegalConsentEnhancer(){
 useEffect(()=>{
  ...
  window.fetch=async(input,init={})=>{
   ...
   if(url.includes('/auth/v1/signup') && init?.body){
    if(!accepted){
     ...
     if(message){message.textContent='Please accept the Terms of Service and Privacy Notice before creating your account.';message.style.display='block';}
     return new Response(JSON.stringify({msg:'Please accept the Terms of Service and Privacy Notice before creating your account.'}),{status:400,headers:{'Content-Type':'application/json'}});
    }
    try{
  ...
  const enhance=()=>{
   ...
   checkbox.addEventListener('change',()=>{accepted=checkbox.checked;const m=document.getElementById(MESSAGE_ID);if(m&&accepted)m.style.display='none';});
