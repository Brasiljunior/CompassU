...
export async function exportInstitutionalAnalyticsPdf({analytics,institution}){
  ...
  const navy=[15,29,64],blue=[47,111,237],violet=[111,92,255],gold=[244,183,64],ink=[23,32,51],muted=[102,112,133],line=[231,234,240],soft=[247,249,252];
  let page=1;
  ...
  const bar=(label,value,max,x,y,w=CONTENT)=>{
    const n=parseFloat(value)||0;const maxN=Math.max(1,Number(max)||1);const bw=Math.max(3,(n/maxN)*(w-142));
  ...
  const tableHeader=(cols,y)=>{
    ...
    setText(8,muted,'bold');cols.forEach(c=>doc.text(c.label,M+c.x,y+17,{align:c.align||'left'}));
    return y+34;
  };

  doc.setFillColor(...navy);doc.rect(0,0,W,H,'F');
  doc.setFillColor(...blue);doc.circle(W-72,74,48,'F');doc.setFillColor(...violet);doc.circle(W-35,117,28,'F');doc.setFillColor(...gold);doc.circle(W-95,145,10,'F');
  ...
  doc.setFillColor(31,48,89);doc.roundedRect(M,380,CONTENT,176,18,18,'F');
  ...
  const strongest=dims.slice().sort((a,b)=>(b.average_score||0)-(a.average_score||0))[0];const topCluster=clusters[0],topMajor=majors[0],topCareer=careers[0];
  const insights=[`CompassU has ${p.assigned_accounts??0} assigned account${p.assigned_accounts===1?'':'s'}, with ${p.completed_students??0} student${p.completed_students===1?'':'s'} completing an assessment (${p.completion_rate??0}% completion).`];
  ...
  insights.forEach((t,i)=>{doc.setFillColor(...soft);doc.roundedRect(M,y,CONTENT,48,8,8,'F');setText(9,blue,'bold');doc.text(`${i+1}`,M+14,y+27);wrapped(t,M+38,y+19,CONTENT-52,10,ink,13);y+=58;});
  ...
  else dims.forEach(d=>{y=bar(labelize(d.dimension),`${d.average_score}%`,100,M,y);setText(7,muted);doc.text(`${d.student_count} students`,M+112,y-11)});
  y+=16;y=section('Top Career Clusters','Dominant cluster distribution across students in the selected scope.',y);
  if(!clusters.length)y=noData('No career cluster reaches the current privacy reporting threshold.',y);
  else{const max=Math.max(...clusters.map(c=>c.student_count||0),1);clusters.slice(0,10).forEach(c=>{y=bar(labelize(c.cluster),c.student_count,max,M,y)})}
  ...
  if(!majors.length)y=noData('No major reaches the current privacy reporting threshold.',y);
    ...
    majors.slice(0,15).forEach(m=>{if(y>720){y=addPage('Top Recommended Majors','INSTITUTIONAL ANALYTICS | CONTINUED');y=tableHeader([{label:'MAJOR',x:10},{label:'STUDENTS',x:382,align:'right'},{label:'AVG. MATCH',x:500,align:'right'}],y)}setText(10,ink,'bold');const name=doc.splitTextToSize(m.major_name||'-',315);doc.text(name,M+10,y+10);if(m.career_cluster){setText(8,muted);doc.text(labelize(m.career_cluster),M+10,y+28+(name.length-1)*11)}setText(10,ink);doc.text(safe(m.student_count??'-'),M+382,y+10,{align:'right'});doc.text(m.average_match_score==null?'-':`${m.average_match_score}%`,M+500,y+10,{align:'right'});const rh=Math.max(42,26+(name.length-1)*11);doc.setDrawColor(...line);doc.line(M,y+rh-6,W-M,y+rh-6);y+=rh});
  }
  ...
  if(!careers.length)y=noData('No career reaches the current privacy reporting threshold.',y);
    ...
    careers.slice(0,18).forEach(c=>{if(y>720){y=addPage('Top Career Alignments','INSTITUTIONAL ANALYTICS | CONTINUED');y=tableHeader([{label:'CAREER',x:10},{label:'SOC',x:365},{label:'STUDENTS',x:500,align:'right'}],y)}setText(10,ink,'bold');const name=doc.splitTextToSize(c.occupation_name||'-',320);doc.text(name,M+10,y+10);setText(9,muted);doc.text(c.soc_code||'-',M+365,y+10);setText(10,ink);doc.text(safe(c.student_count??'-'),M+500,y+10,{align:'right'});const rh=Math.max(34,22+(name.length-1)*11);doc.setDrawColor(...line);doc.line(M,y+rh-5,W-M,y+rh-5);y+=rh});
  }
  ...
  methods.forEach(([k,v])=>{if(y>700)y=addPage('Methodology & Data Definitions','INSTITUTIONAL ANALYTICS | CONTINUED');setText(10,blue,'bold');doc.text(k,M,y);y=wrapped(v,M,y+17,CONTENT,10,ink,14);y+=18});
  if(y<650){y+=8;doc.setFillColor(...navy);doc.roundedRect(M,y,CONTENT,72,10,10,'F');setText(11,[255,255,255],'bold');doc.text('CompassU | Institutional Intelligence',M+16,y+26);setText(9,[205,216,244]);doc.text('Aggregate insight for advising, pathway planning, student success, and academic strategy.',M+16,y+48)}
