...
export default function AdminAnalyticsPanel(){
  ...
  useEffect(()=>{let activeToken='';const syncSession=()=>{const s=readSession();const token=s?.access_token||'';if(token===activeToken)return;activeToken=token;if(!token){setSession(null);setAnalytics(null);setComparison(null);return}setSession(s);loadInstitutions(s);loadAnalytics(s,'','','')};const refreshInstitutions=()=>{const s=readSession();if(s?.access_token)loadInstitutions(s)};syncSession();const timer=setInterval(syncSession,500);const onStorage=e=>{if(e.key==='compassu_session')syncSession()};window.addEventListener('storage',onStorage);window.addEventListener('compassu:institutions-updated',refreshInstitutions);return()=>{clearInterval(timer);window.removeEventListener('storage',onStorage);window.removeEventListener('compassu:institutions-updated',refreshInstitutions)}},[]);
