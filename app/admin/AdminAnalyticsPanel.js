...
export default function AdminAnalyticsPanel(){
  const[session,setSession]=useState(null),[analytics,setAnalytics]=useState(null),[institutions,setInstitutions]=useState([]),[institution,setInstitution]=useState(''),[startDate,setStartDate]=useState(''),[endDate,setEndDate]=useState(''),[busy,setBusy]=useState(false),[exporting,setExporting]=useState(false),[error,setError]=useState('');
  const headers=current=>({apikey:SUPABASE_KEY,'Content-Type':'application/json',Authorization:`Bearer ${current?.access_token||''}`});

  async function loadInstitutions(current){try{const r=await fetch(`${SUPABASE_URL}/rest/v1/account_institutions?select=institution&institution=not.is.null`,{headers:headers(current)});const rows=await r.json();if(!r.ok)throw new Error(rows?.message||rows?.error||'Unable to load institutions');setInstitutions([...new Set((rows||[]).map(x=>String(x.institution||'').trim()).filter(Boolean))].sort())}catch(e){console.warn('Institution list unavailable',e)}}

  async function loadAnalytics(current=session,nextInstitution=institution,nextStart=startDate,nextEnd=endDate){
  ...
  function clearPeriod(){setStartDate('');setEndDate('');loadAnalytics(session,institution,'','')}
  async function exportReport(){if(!analytics||exporting)return;setExporting(true);setError('');try{await exportInstitutionalAnalyticsPdf({analytics,institution,startDate,endDate})}catch(e){setError(e?.message||'Unable to generate the institutional analytics report.')}finally{setExporting(false)}}

  useEffect(()=>{let activeToken='';const syncSession=()=>{const s=readSession();const token=s?.access_token||'';if(token===activeToken)return;activeToken=token;if(!token){setSession(null);setAnalytics(null);return}setSession(s);loadInstitutions(s);loadAnalytics(s,'','','')};syncSession();const timer=setInterval(syncSession,500);const onStorage=e=>{if(e.key==='compassu_session')syncSession()};window.addEventListener('storage',onStorage);return()=>{clearInterval(timer);window.removeEventListener('storage',onStorage)}},[]);
  ...
  return <section className="analyticsPanel adminPanel"><div className="analyticsHead"><div><div className="adminKicker">INSTITUTIONAL INTELLIGENCE</div><h2>Institutional Analytics</h2><p>Aggregate participation, student profile, career-cluster, major, and career insights based on each student's latest completed CompassU assessment.</p></div><div className="analyticsControls">
    <select value={institution} onChange={e=>{const v=e.target.value;setInstitution(v);loadAnalytics(session,v,startDate,endDate)}}><option value="">All Institutions</option>{institutions.map(v=><option key={v} value={v}>{v}</option>)}</select>
    <label className="analyticsDate"><span>From</span><input type="date" value={startDate} max={endDate||undefined} onChange={e=>setStartDate(e.target.value)}/></label>
    <label className="analyticsDate"><span>Through</span><input type="date" value={endDate} min={startDate||undefined} onChange={e=>setEndDate(e.target.value)}/></label>
    <button className="btn ghost" disabled={busy} onClick={()=>loadAnalytics()}>{busy?'Applying…':'Apply Period'}</button>
    {(startDate||endDate)&&<button className="btn ghost" disabled={busy} onClick={clearPeriod}>Clear Period</button>}
  ...
  {analytics?.reporting_period?.filtered&&<div className="analyticsPeriod">Reporting period: <b>{analytics.reporting_period.start_date||'Beginning of records'}</b> through <b>{analytics.reporting_period.end_date||'Present'}</b>. Assessment metrics reflect activity within this period.</div>}
  ...
  </div><div className="analyticsPrivacy">Privacy safeguard: categories representing fewer than {analytics?.privacy?.small_cell_threshold??5} students are suppressed. Analytics use each student's latest completed assessment within the selected reporting period.</div></>}
  </section>
}
