  ...
  return <section className="analyticsPanel adminPanel">
    ...
    {analytics&&<>
      ...
      <div className="analyticsGrid">
        ...
        <div className="analyticsCard"><h3>Top Recommended Majors</h3><p className="analyticsSub">Based on each student's #1 CompassU major match.</p><table className="analyticsTable"><thead><tr><th>Major</th><th>Students</th><th>Avg. Match</th></tr></thead><tbody>{majors.slice(0,10).map(m=><tr key={m.major_id}><td><b>{m.major_name}</b>{m.career_cluster&&<span>{labelize(m.career_cluster)}</span>}</td><td>{m.student_count}</td><td>{m.average_match_score==null?'—':`${m.average_match_score}%`}</td></tr>)}</tbody></table>{!majors.length&&<div className="adminEmpty">No reportable major cells yet.</div>}</div>
        <div className="analyticsCard"><h3>Top Career Alignments</h3><p className="analyticsSub">Career alignment derived from each student's top major and strongest occupation relationship.</p><table className="analyticsTable"><thead><tr><th>Career</th><th>Students</th></tr></thead><tbody>{careers.slice(0,10).map(c=><tr key={c.occupation_id}><td><b>{c.occupation_name}</b>{c.soc_code&&<span>SOC {c.soc_code}</span>}</td><td>{c.student_count}</td></tr>)}</tbody></table>{!careers.length&&<div className="adminEmpty">No reportable career cells yet.</div>}</div>
      </div>
