  ...
  useEffect(()=>{
    ...
    window.fetch=async(input,init)=>{
      ...
      try{
        ...
        if(action==='overview'&&response.ok){const body=await response.clone().json();await loadPersistentInstitutions();overviewUsers=(body?.users||[]).map(user=>({...user,institution:normalize(user?.institution||user?.profile?.institution||user?.user_metadata?.institution||institutionCache[String(user?.email||'').toLowerCase()])}));scheduleEnhance()}
      }catch{}
      return response;
    };
    ...
    function getInstitutionForRow(row){const email=getEmailForRow(row);const user=overviewUsers.find(u=>String(u.email||'').toLowerCase()===email);return normalize(user?.institution||institutionCache[email])||'—'}
