  ...
  useEffect(()=>{
    ...
    function renderRow(row,user){
      ...
      if(!created||!lastSignIn||!survey||!access||!actions){
        ...
        created=base[1]||created;lastSignIn=base[2]||lastSignIn;survey=base[3]||survey;access=base[4]||access;actions=base[5]||actions;
        if(created)created.dataset.compassuOriginal='created';if(lastSignIn)lastSignIn.dataset.compassuOriginal='last-sign-in';if(survey)survey.dataset.compassuOriginal='survey-status';if(access)access.dataset.compassuOriginal='access';if(actions)actions.dataset.compassuOriginal='actions';
      }
    ...
    async function refresh(force=false){
      ...
      const state=readState();const signature=JSON.stringify([state.page||1,state.page_size||50,state.search||'',state.institution||'']);
      ...
      try{
        ...
        const body=await response.json().catch(()=>({}));if(response.ok){currentUsers=Array.isArray(body?.users)?body.users:[];lastSignature=signature;paint()}
    ...
    const onRefresh=()=>schedule(true);window.addEventListener('compassu:institutions-updated',onRefresh);window.addEventListener('compassu-admin-50k-refresh',onRefresh);
    const interval=setInterval(()=>{paint();refresh(false)},1200);schedule(true);
