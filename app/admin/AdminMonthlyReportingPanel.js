...
export default function AdminMonthlyReportingPanel(){
  ...
  async function removeRecipient(profile,email){
    if(!session?.access_token||busy)return;
    const remaining=(profile.recipient_emails||[]).filter(value=>String(value).toLowerCase()!==String(email).toLowerCase());
    const deleting=!remaining.length;
    if(!window.confirm(deleting?`Remove ${email} and permanently delete the ${profile.institution} monthly reporting configuration?`:`Remove ${email} from ${profile.institution} monthly reports?`))return;
    setBusy(true);setError('');setNotice('');
    try{
      const r=deleting
        ?await fetch(`${SUPABASE_URL}/rest/v1/rpc/delete_institution_monthly_reporting_profile`,{method:'POST',headers:headers(session),body:JSON.stringify({p_institution:profile.institution})})
        :await fetch(`${SUPABASE_URL}/rest/v1/rpc/set_institution_monthly_reporting_profile`,{method:'POST',headers:headers(session),body:JSON.stringify({p_institution:profile.institution,p_recipient_emails:remaining,p_active:profile.active,p_delivery_day:profile.delivery_day||5,p_timezone:profile.timezone||'America/Chicago',p_include_institutional_analytics:profile.include_institutional_analytics!==false,p_include_institutional_trends:profile.include_institutional_trends!==false,p_include_executive_insights:profile.include_executive_insights!==false})});
      const body=await r.json();if(!r.ok)throw new Error(body?.message||body?.hint||'Unable to remove monthly report recipient.');
      if(form.institution===profile.institution){if(deleting)clear();else setForm(current=>({...current,emails:remaining.join(', ')}))}
      setNotice(deleting?`${profile.institution} monthly reporting configuration was permanently deleted.`:`${email} was removed from ${profile.institution} monthly reports.`);
      await load(session);
    }catch(e){setError(e.message||'Unable to remove monthly report recipient.')}finally{setBusy(false)}
  }

  async function deleteProfile(profile){
    if(!session?.access_token||busy)return;
    if(!window.confirm(`Permanently delete the ${profile.institution} monthly reporting configuration? Delivery history will be preserved.`))return;
    setBusy(true);setError('');setNotice('');
    try{
      const r=await fetch(`${SUPABASE_URL}/rest/v1/rpc/delete_institution_monthly_reporting_profile`,{method:'POST',headers:headers(session),body:JSON.stringify({p_institution:profile.institution})});
      const body=await r.json();if(!r.ok)throw new Error(body?.message||body?.hint||'Unable to delete monthly reporting configuration.');
      if(form.institution===profile.institution)clear();
      setNotice(`${profile.institution} monthly reporting configuration was permanently deleted.`);await load(session);
    }catch(e){setError(e.message||'Unable to delete monthly reporting configuration.')}finally{setBusy(false)}
  }
  ...
  return <section className="analyticsPanel adminPanel"><div className="analyticsHead"><div><div className="adminKicker">STAGE 2E · AUTOMATED REPORTING</div><h2>Monthly Institutional Reporting</h2><p>Configure automatic delivery of the prior month's Institutional Analytics, Institutional Trends, and Executive Institutional Insights PDFs. Institutions receive reports by email without administrator access.</p></div><button className="btn ghost" disabled={busy} onClick={()=>load()}>{busy?'Refreshing…':'Refresh'}</button></div>
    ...
    <div className="analyticsGrid">
      ...
      <div className="analyticsCard"><h3>Configured Institutions</h3><p className="analyticsSub">Only the CompassU Master Administrator can manage this delivery list. Removing the final recipient deletes the configuration; delivery history remains available.</p>{profiles.length?<div className="adminTableWrap"><table className="analyticsTable"><thead><tr><th>Institution</th><th>Recipients</th><th>Day</th><th>Status</th><th>Actions</th></tr></thead><tbody>{profiles.map(p=><tr key={p.id} onClick={()=>edit(p)} style={{cursor:'pointer'}}><td><b>{p.institution}</b><span>{[p.include_institutional_analytics&&'Analytics',p.include_institutional_trends&&'Trends',p.include_executive_insights&&'Executive'].filter(Boolean).join(' · ')}</span></td><td><div className="monthlyRecipients">{(p.recipient_emails||[]).length?(p.recipient_emails||[]).map(email=><div key={email}><span>{email}</span><button type="button" disabled={busy} onClick={event=>{event.stopPropagation();removeRecipient(p,email)}}>Remove</button></div>):<span>No recipients</span>}</div></td><td>{p.delivery_day}</td><td><span className={`adminStatus ${p.active?'active':'suspended'}`}>{p.active?'Active':'Paused'}</span></td><td><button type="button" className="monthlyDelete" disabled={busy} onClick={event=>{event.stopPropagation();deleteProfile(p)}}>Delete Configuration</button></td></tr>)}</tbody></table></div>:<div className="adminEmpty">No monthly reporting profiles configured yet.</div>}</div>
    </div>
