...
export default function AdminMonthlyReportingPanel(){
  ...
  async function removeRecipient(profile,email){
    if(!session?.access_token||busy)return;
    if(!window.confirm(`Remove ${email} from ${profile.institution} monthly reports?`))return;
    const remaining=(profile.recipient_emails||[]).filter(value=>String(value).toLowerCase()!==String(email).toLowerCase());
    setBusy(true);setError('');setNotice('');
    try{
      const r=await fetch(`${SUPABASE_URL}/rest/v1/rpc/set_institution_monthly_reporting_profile`,{method:'POST',headers:headers(session),body:JSON.stringify({p_institution:profile.institution,p_recipient_emails:remaining,p_active:remaining.length?profile.active:false,p_delivery_day:profile.delivery_day||5,p_timezone:profile.timezone||'America/Chicago',p_include_institutional_analytics:profile.include_institutional_analytics!==false,p_include_institutional_trends:profile.include_institutional_trends!==false,p_include_executive_insights:profile.include_executive_insights!==false})});
      const body=await r.json();if(!r.ok)throw new Error(body?.message||body?.hint||'Unable to remove monthly report recipient.');
      if(form.institution===profile.institution)setForm(current=>({...current,emails:remaining.join(', '),active:remaining.length?current.active:false}));
      setNotice(`${email} was removed from ${profile.institution} monthly reports.${remaining.length?'':' Monthly reporting was paused because no recipients remain.'}`);
      await load(session);
