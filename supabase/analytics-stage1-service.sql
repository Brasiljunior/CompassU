...
begin
  ...
  -- Institutional major demand is based on whether a major appears anywhere in a student's
  -- Top 10 CompassU recommendations, not only on the single #1 recommendation.
  ...
  major_distribution as (
    select major_id,name,career_cluster,
    ...
    from student_major_recommendations
    group by major_id,name,career_cluster
  ),
  -- Career alignment uses each student's Top 3 majors so institutional reports reflect
  -- recurring career pathways rather than only one occupation attached to one #1 major.
  student_career_candidates as (
    select smr.user_id,o.id as occupation_id,o.name,o.soc_code,
      row_number() over(
        partition by smr.user_id,o.id
        order by smr.rank asc, mo.relevance_weight desc, o.name
      ) as occupation_dedupe_rank
  ...
  career_distribution as (
    select occupation_id,name,soc_code,count(distinct user_id)::int as student_count
    from student_career_candidates
    where occupation_dedupe_rank=1
    group by occupation_id,name,soc_code
  ),
  institution_list as (
  ...
  select jsonb_build_object(
    ...
    'privacy', jsonb_build_object(
      ...
      'major_method','top_10_recommendation_frequency',
      'career_method','career_frequency_from_top_3_majors'
    ),
    'participation', (
      select jsonb_build_object(
    ...
    'dimensions', coalesce((select jsonb_agg(jsonb_build_object(
      'dimension',dimension,
      'student_count',case when student_count<v_threshold then null else student_count end,
      'average_score',case when student_count<v_threshold then null else average_score end,
      'suppressed',student_count<v_threshold
    ) order by dimension) from dimension_summary),'[]'::jsonb),
    'career_clusters', coalesce((select jsonb_agg(jsonb_build_object(
      'cluster',case when student_count<v_threshold then 'Suppressed' else cluster end,
      'student_count',case when student_count<v_threshold then null else student_count end,
      'suppressed',student_count<v_threshold
    ...
    'top_majors', coalesce((select jsonb_agg(jsonb_build_object(
      'major_id',case when student_count<v_threshold then null else major_id end,
      'major_name',case when student_count<v_threshold then 'Suppressed' else name end,
      'career_cluster',case when student_count<v_threshold then null else career_cluster end,
      'student_count',case when student_count<v_threshold then null else student_count end,
      'average_match_score',case when student_count<v_threshold then null else average_match_score end,
      'average_rank',case when student_count<v_threshold then null else average_rank end,
      'suppressed',student_count<v_threshold
    ) order by student_count desc,name) from major_distribution),'[]'::jsonb),
    'top_careers', coalesce((select jsonb_agg(jsonb_build_object(
      'occupation_id',case when student_count<v_threshold then null else occupation_id end,
      'occupation_name',case when student_count<v_threshold then 'Suppressed' else name end,
      'soc_code',case when student_count<v_threshold then null else soc_code end,
      'student_count',case when student_count<v_threshold then null else student_count end,
      'suppressed',student_count<v_threshold
    ) order by student_count desc,name) from career_distribution),'[]'::jsonb),
...
'CompassU aggregate institutional analytics. Admin-only, latest completed assessment per student, major frequency across Top 10 recommendations, career frequency from Top 3 majors, small-cell suppression threshold 5.';
